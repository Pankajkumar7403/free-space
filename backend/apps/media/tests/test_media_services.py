...
from apps.media.services import (
    PresignedUploadResult, confirm_upload,
    create_media_record, update_media_status,
)
...
class TestCreateMediaRecord:
    ...
    def test_creates_media_and_returns_presigned_url(self, mock_url, db, user):
        result = create_media_record(owner=user, mime_type="image/jpeg", file_size=1024 * 100)
    ...
    def test_rejects_oversized_image(self, db, user):
        ...
        with pytest.raises(FileTooLargeError):
            create_media_record(owner=user, mime_type="image/jpeg", file_size=25 * 1024 * 1024)
...
class TestUpdateMediaStatus:
    def test_sets_ready_status(self, db, user):
        ...
        result = update_media_status(
            media_id=str(media.pk), status=MediaStatus.READY,
