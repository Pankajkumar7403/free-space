...
class TestComputeScore:
    ...
    def test_engagement_boosts_score(self):
        now = self._now()
        low  = compute_score(post_created_at=now - timedelta(hours=1), like_count=0,   now=now)
        high = compute_score(post_created_at=now - timedelta(hours=1), like_count=500, now=now)
        assert high > low

    def test_mutual_follow_boosts_score(self):
        now = self._now()
        one_way = compute_score(post_created_at=now - timedelta(hours=1), is_mutual_follow=False, now=now)
        mutual  = compute_score(post_created_at=now - timedelta(hours=1), is_mutual_follow=True,  now=now)
    ...
    def test_score_is_between_zero_and_one(self):
        ...
        for hours_ago in [0, 1, 6, 24, 72, 168]:
            ...
            assert 0.0 <= score <= 1.0, f"score={score} out of range for hours_ago={hours_ago}"
