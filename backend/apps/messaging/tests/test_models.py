...
class TestConversationModel:
    ...
    def test_str_direct_conversation(self, conversation_factory, user_factory):
        conv = conversation_factory(conversation_type=ConversationType.DIRECT, created_by=user_factory())
    ...
    def test_is_group_property(self, conversation_factory, user_factory):
        conv = conversation_factory(conversation_type=ConversationType.GROUP, name="Group", created_by=user_factory())
...
class TestConversationParticipantModel:
    def test_unique_together_user_conversation(self, conversation_factory, user_factory, participant_factory):
        user = user_factory()
        conv = conversation_factory(created_by=user)
        participant_factory(conversation=conv, user=user)
        with pytest.raises(Exception):
            ConversationParticipant.objects.create(conversation=conv, user=user, role=ParticipantRole.MEMBER)
...
class TestMessageReactionModel:
    def test_unique_together_message_user_emoji(self, reaction_factory, message_factory, conversation_factory, user_factory):
        msg = message_factory(conversation=conversation_factory(created_by=user_factory()), sender=user_factory())
        user = user_factory()
