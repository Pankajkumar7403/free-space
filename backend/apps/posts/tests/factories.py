...
class MediaFactory(BaseFactory):
    ...
    file_size = 1024 * 100
    original_url = factory.Sequence(
        lambda n: f"https://cdn.example.com/originals/{n}.jpg"
    )
    processed_url = factory.Sequence(
        lambda n: f"https://cdn.example.com/processed/{n}.jpg"
    )
    thumbnail_url = factory.Sequence(
        lambda n: f"https://cdn.example.com/thumbnails/{n}.jpg"
    )
    width = 1080
    height = 1080
    ...
    class Params:
        video = factory.Trait(
            media_type=MediaType.VIDEO, mime_type="video/mp4", duration=15.0
        )
