...
def get_comment_by_id(comment_id) -> Comment:
    ...
    try:
        return Comment.objects.select_related("author", "post", "parent").get(
...
def get_top_level_comments(*, post_id) -> QuerySet:
    ...
    """
    return (
        Comment.objects.filter(post_id=post_id, parent=None, is_deleted=False)
...
def get_replies(*, parent_id) -> QuerySet:
    ...
    return (
        Comment.objects.filter(parent_id=parent_id, is_deleted=False)
        .select_related("author")
        .order_by("created_at")
    )
